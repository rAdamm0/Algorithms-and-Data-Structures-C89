 ...
 *
 * Copyright (C) 2021--2025 Nicolas Farabegoli, Moreno Marzolla
 *
...
ricorrenza, ad esempio usando il Master Theorem oppure sommando i
costi lungo l'albero della ricorsione, si ottiene $T(n) = \Theta(n
\log n)$.

L'algoritmo Merge Sort può essere realizzato in modo ricorsivo o
iterativo; la soluzione che verrà proposta è di tipo ricorsivo, ma chi
lo desidera può implementare l'algoritmo in forma iterativa.
...
È possibile usare la funzione di libreria `qsort()` per ordinare un
array contente elementi di tipo qualunque; vediamo qui un esempio per
ordinare un array di interi.

Occorre innanzitutto definire una funzione per confrontare tra loro
coppie di elementi da ordinare; chiamiamo `cmp_increasing()` tale
funzione (è possibile scegliere un nome qualunque). La funzione deve
accettare due parametri di tipo `const void*` e restituire un
intero. Per effettuare il confronto, la funzione deve conoscere il
vero tipo degli elementi dell'array da ordinare, per cui serve una
funzione diversa in base al tipo di dato. Nel nostro caso, i puntatori
`a` e `b` vanno convertiti a puntatori a _interi costanti_, per poi
essere internamente dereferenziati per confrontare il valore puntato.
...
puntato. Per comprendere la dichiarazione delle variabili `va` e `vb`
conviene ragionare per passi:
...
*)a` produce un _warning_, perché ha come valore un puntatore _non
costante_. Questo violerebbe il vincolo originale che richiede che il
valore puntato da `a` non possa essere modificato.
...
grandi (es., 100000 elementi o più), qual è il tempo di esecuzione nei
seguenti casi?
...
void merge_sort(int *v, int p, int r, int *buffer)
{
    if(p<r){
        int q = (p+r)/2;
        merge_sort(v,p,q,buffer);
        merge_sort(v,q+1,r,buffer);
