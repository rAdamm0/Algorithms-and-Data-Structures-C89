...
(`.`). La configurazione iniziale contiene un'unica stella in
posizione centrale (cella 4):
...
un buco nero; inoltre, i suoi frammenti vanno a finire in alcune delle
celle adiacenti, trasformando i buchi neri in stelle e le stelle in
buchi neri. Il vicinato è definito in modo differente per ogni cella,
come nella Figura 1.

![Figura 1: vicinato delle nove celle.](shooting-stars.svg)

Come si vede, il vicinato della cella 0 è costituita da (1, 3, 4); il
...
lettura, solo le celle contenenti una stella sono indicate con il
rispettivo numero):
...
int board[3][3]; /* 0 indica un buco nero, 1 una stella */
...
/* Inizializza la griglia alla configurazione iniziale con una singola
   stella al centro. */
    ...
    for (i=0; i<3; i++) {
        for (j=0; j<3; j++) {
            if (i == 1 && j == 1)
                ...
                board[i][j] = 0;
            /* La condizione precedente può essere semplificata in una
               singola istruzione:
...
/* Fa esplodere la stella in posizione `k`; le posizioni sono numerate
   ...
   configurazione è quella vincente, `LOST` se la nuova configurazione
   è quella perdente (solo buchi neri), e `OK` se non è né vincente né
   perdente.
*/
Move shoot(int k)
{
